...
def main(args):
    ...
    srcImg = cv2.blur(srcImg,(5,5))

    srcImg_gray = cv2.imread(image_path_list[0],0)
    srcImg_gray = cv2.blur(srcImg_gray,(5,5))  

    trgImg = cv2.imread(image_path_list[1])
    trgImg = cv2.blur(trgImg,(5,5))

    trgImg_gray = cv2.imread(image_path_list[1],0)
    trgImg_gray = cv2.blur(trgImg_gray,(5,5))
    ...
    # cv2.imwrite('newSwap.png', srcImg)
