...
def deepSwap(args, srcImg, srcImg_gray, trgImg, trgImg_gray, name):
	# if args.isShow or args.isTexture:
	#     from utils.cv_plot import plot_kpt, plot_vertices, plot_pose_box
	...
	# print(trgImg.shape)
	# print(trgImg_gray.shape)
	# print(srcImg.shape)
	# print(srcImg_gray.shape)
	# print(srcVert.dtype)
	# newSwap = trgImg.copy()
	# for pix in range(trgVert.shape[0]):
	#     newSwap[trgVert[pix,1], trgVert[pix,0],:] = srcImg[srcVert[pix,1], srcVert[pix,0],:]
		# srcImg[srcVert[pix,1], srcVert[pix,0],:] = [255,255,255]
	...
	# cv2.imwrite('newSwap.png', newSwap)
	return newSwap
...
parser.add_argument('--swap_video', default=False, type=ast.literal_eval,
					help='swap 2 largest faces within a video')

args = parser.parse_args()
src = cv2.imread("TestImages/Scarlett.jpg")
srcGray = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
# path = "TestImages/Test3.mp4"
path = "a.mp4"
cap = cv2.VideoCapture(path)
fourcc = cv2.VideoWriter_fourcc(*'MJPG')
ret, trg= cap.read()
vw = cv2.VideoWriter("Data1OutputPRNet.avi", fourcc, 30, (trg.shape[1], trg.shape[0]))
frameCount= 1
srcSwap =True
...
while (cap.isOpened()):
	...
	name = str(frameCount)
	# swapImg = deepSwap(args, src, srcGray, trg, trgGray, name)
	if(args.swap_video):
		...
		if len(kpt)>1 and len(vert)>1:
			...
			newSwap = swap(newSwap0, trg.copy(), newSwap0Gray, srcNewGray.copy(), 
					...
					debug = False, showW = 240, showH = 220)
			cv2.imwrite("res/1.jpg",newSwap0)
			cv2.imwrite("res/2.jpg",newSwap)
			vw.write(newSwap)
		else:
			vw.write(trg)
	else:
