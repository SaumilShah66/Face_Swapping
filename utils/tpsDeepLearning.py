...
def NewWarp1(img_target, img_src, trg_points, src_points, weights, mask_src, target_points):
	xyTrg_min = np.float32([min(trg_points[:,0]),min(trg_points[:,1])])
	xyTrg_max = np.float32([max(trg_points[:,0]),max(trg_points[:,1])])
	xySrc_min = np.float32([min(src_points[:,0]),min(src_points[:,1])])
	xySrc_max = np.float32([max(src_points[:,0]),max(src_points[:,1])])
	...
	for i in range(xy.shape[0]):
		u[i] = fxy(xy[i,:], target_points, weights[:,0])
		v[i] = fxy(xy[i,:], target_points, weights[:,1])
...
def swap(targetImage, sourceImage, targetImage_gray, sourceImage_gray, 
	...
	cv2.imwrite("res/targetMask.jpg", mask_target_)
	# mask_target_ = mask_from_points((w, h), trgPtsall)
	# plt.imshow(mask_target_)
	w, h = sourceImage_gray.shape
	mask_source_ = mask_from_points((w, h), srcPtsall)
	cv2.imwrite("res/sourceMask.jpg", mask_source_)
	weights = NewTps(target_points, source_points, 0.001)
	
	# tt = NewWarp1(sourceImage, targetImage, trgPtsall, srcPtsall,weights, mask_source_, target_points)

	tt = NewWarp1(targetImage,sourceImage, trgPtsall, srcPtsall,weights, mask_source_, target_points)
	srcBox = cv2.boundingRect(mask_target_)
	(x,y,w,h) = srcBox
	center = ((x+x+w)/2,(y+y+h)/2)
	cv2.imwrite("res/MaskDeepLearning.jpg", mask_target_)
	m = np.where(mask_target_!=0,targetImage_gray, 0)
	n = np.where(mask_source_!=0,sourceImage_gray, 0)
	cv2.imwrite("res/TM.jpg",m)
	cv2.imwrite("res/SM.jpg",n)
	...
	# newWarped = ColorBlend(targetImage, tt, mask_target_)
	if debug:
